        ...
        match self.mode:
            ...
            case "FMRMode":
                ...
                for point in self.points:
                    ...
                    self.emit('current_point', point)
                    self.counter = self.counter + 1
                    window.inputs.single_meas_duration.setValue(time()-start)
                    if self.should_stop():
                        log.warning("Caught the stop flag in the procedure")
                        break
                self.fmrmode.end()
