...
#################################################################### PARAMETERS #####################################################################
    ...
    set_pulsegenerator=ListParameter("Pulse Generator", choices=["Agilent 2912","Tektronix 10,070A", "none"], default = parameters_from_file["set_pulsegenerator"], group_by="mode", group_condition=lambda v: v=="CIMSMode")
    ...
    address_relay=ListParameter("Relay address", default = parameters_from_file["address_relay"] if parameters_from_file["address_relay"] in finded_instruments else 'None', choices=finded_instruments, group_by = {"mode": lambda v: v=="CIMSMode", "set_pulsegenerator": lambda v: v == "Tektronix 10,070A"})
    ...
    delay_measurement = FloatParameter("Delay Measurement", default = parameters_from_file["delay_measurement"], units="s", group_by={"mode": lambda v: v == "CIMSMode"})
    ...
    def startup(self):
        ...
        match self.mode:
            ...
            case "CIMSMode":
                self.CIMSmode = CIMSMode(self.vector, self.mode_cims_relays, self.sourcemeter_bias, self.set_sourcemeter, self.set_multimeter,self.set_pulsegenerator, self.set_gaussmeter, self.set_field, self.set_automaticstation, self.set_switch, self.set_kriostat, self.set_rotationstation, self.address_sourcemeter, self.address_multimeter,self.address_pulsegenerator, self.address_gaussmeter, self.address_switch, self.delay_field, self.delay_measurement, self.delay_bias, self.sourcemter_source, self.sourcemeter_compliance, self.sourcemeter_channel, self.sourcemeter_limit, self.sourcemeter_nplc, self.sourcemeter_average, self.multimeter_function, self.multimeter_resolution, self.multimeter_autorange, self.multimeter_range, self.multimeter_average, self.field_constant, self.gaussmeter_range, self.gaussmeter_resolution, self.multimeter_nplc, self.address_daq, self.field_step, self.address_rotationstation, self.constant_field_value,self.rotation_axis, self.rotation_polar_constant, self.rotation_azimuth_constant,self.pulsegenerator_duration,self.pulsegenerator_offset,self.pulsegenerator_pulsetype,self.pulsegenerator_channel,self.set_relay,self.address_relay)
...
-podpiac zahardkodowane zmiene by zmieniac je w programie (kanaly tez gdy agilent 2912) - ok 
-wywala err na agilent 2912 przy funkcji init ale idzie dalej
-scalic sterowniki agilenta z tektronixem aby byly te same funkcje
-doda zmienna "czas oczekiwania" po impulsie by ja edytowac w programie - ok
-USE internal relays dodac do sterownikow i oprogramowac w programie (generator agilent i sourcemeter keithley) -ok

HIGH_Z - to jest ta komenda do keithleya by otlaczal output 
